...
Total Receipt must always equal
...
Closing Balance must always equal
...
The worksheet included information such as
...
The workbook was imported into Python using **Pandas**.

The first few rows were inspected to understand the worksheet structure.
...
```text
Calculated Total Receipt
```

with

```text
Reported Total Receipt
```
...
```python
Total Receipt Error =
Reported Total Receipt != Calculated Total Receipt

Closing Balance Error =
...
During analysis, almost every row appeared incorrect.

This happened because the uploaded workbook did **not** contain separate **Receipt** and **Interest** columns.

Instead, it already contained a **Total Receipt** column.
...
which effectively added the Opening Balance twice.

Therefore, the calculated values became larger than the reported values.

This highlighted an important lesson.

## AI can only validate calculations when all required input columns are available.

Without separate **Receipt** and **Interest** columns, the first validation rule cannot be applied correctly.
...
The next step attempted to create a corrected workbook.

The proposed workflow was

1. Calculate the correct Total Receipt.
2. Calculate the correct Closing Balance.
3. Replace incorrect values.
4. Export a new Excel workbook.

Unfortunately, the execution environment became unavailable before the corrected workbook could be generated.
...
To highlight incorrect rows automatically,
