...
* **Manufacturer** → **Wholesaler** → **Distributor** → **Retailer** → **Consumer (B2C)**
...
Inward Supply means every purchase received by a registered person. It includes:
...
Outward Supply means every sale made by the taxpayer. It includes:
...
`Sales Register` → `GSTR-1` → `GSTR-3B` → `Financial Statements` → `Income Tax Return`
...
* **Sale:** Finished Goods `₹2,00,000` + GST @ 18% (`₹36,000`)
* **Tax Payable:** Output GST (`₹36,000`) - Less ITC (`₹18,000`) = **Net GST Payable (`₹18,000`)**
...
1. **Registered Supplier:** Supplier must possess a valid GST registration.
2. **Valid Tax Invoice:** Invoice should contain GSTIN, Invoice Number/Date, HSN/SAC, Taxable Value, and GST Amount.
3. **Goods or Services Received:** The recipient must actually receive the goods or services.
4. **Supplier Must Pay GST:** Supplier should upload the invoice and pay tax (must appear in GSTR-2B).
5. **Return Filing:** Recipient should file GSTR-3B.
...
* **Manufacturer Purchase:** Raw Material = `₹100`
* **Manufacturer Sale:** Finished Product = `₹150`
* **Value Added:** `₹50`
...
Contains details of outward supplies, including:
...
* **Raw Material GST:** `18%`
* **Finished Goods GST:** `5%`
* **Result:** Unused ITC accumulates.
...
* Incorrect Place of Supply or HSN/SAC codes
* Delayed GST payment or unpaid interest
